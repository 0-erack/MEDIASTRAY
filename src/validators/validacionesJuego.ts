...
export const validarCreacionJuego = (data:any) : Partial<Juego> => {
    ...
    if (data.tags != undefined && (!Array.isArray(data.generos) || !comalista(data.tags))) throw new Error("Invalid tag list");
    ...
    if (Array.isArray(data.genres)) data.genres = data.genres.join(",");
    if (Array.isArray(data.tags)) data.tags = data.tags.join(",");
    if (Array.isArray(data.languages)) data.languages = data.languages.join(",");
    if (Array.isArray(data.price)) data.price = data.price.join(",");
    return {titulo: data.title.trim(), urlPortada1: data.cover1.trim(), urlPortada2: data.cover2.trim(), urlPortada3: data.cover3.trim(), publico: data.public, versionActual: data.version.trim(), descripcion: data.description, descripcionCorta: data.shortDescription.trim(), precio: data.price.trim(), generos: data.genres, tags: data.tags, idiomas: data.languages, avisos: data.warnings}
}
...
export const validarEdicionJuego = (data:any) : Record<string, any> => {
    ...
    if (data.cover2 != undefined && !url(data.cover2)) throw new Error("Invalid game cover 2");
    if (data.cover3 != undefined && !url(data.cover3)) throw new Error("Invalid game cover 3");
    if (typeof data.public !== "boolean" && typeof data.public != undefined) throw new Error("Invalid publicness");
    if (data.version != undefined && !version(data.version)) throw new Error("Invalid game version");
    if (data.description != undefined && !descripcionJuego(data.description)) throw new Error("Invalid description");
    if (data.shortDescription != undefined && !descripcionCortaJuego(data.shortDescription)) throw new Error("Invalid short description");
    if (data.genres != undefined && (!Array.isArray(data.genres) || !comalista(data.genres))) throw new Error("Invalid genre list");
...
export const validarJuego = (data:Juego) : Juego => {
    ...
    if (data.tags != undefined && (!Array.isArray(data.generos) || !comalista(data.tags))) throw new Error("Invalid tag list");
    if (data.idiomas != undefined && (!Array.isArray(data.generos) || !comalista(data.idiomas))) throw new Error("Invalid language list");
    if (data.avisos != undefined && (!Array.isArray(data.generos) || !comalista(data.avisos))) throw new Error("Invalid warnings list");
...
export const formatearJuegoPublico = (data:Partial<Juego>): Record<string, any> => {
    if (typeof data.avisos == "string") data.avisos = data.avisos!.split(",").map(e => e.trim());
    if (typeof data.tags == "string") data.tags = data.tags!.split(",").map(e => e.trim());
    if (typeof data.idiomas == "string") data.idiomas = data.idiomas!.split(",").map(e => e.trim());
    if (typeof data.generos == "string") data.generos = data.generos!.split(",").map(e => e.trim());
    return {id: data.id, owner: data.idCreador, publishDate: data.fechaCreacion, updateDate: data.fechaUltima, followers: data.cantidadSeguidores, players: data.cantidadJugadores, comments: data.cantidadComentarios, title: data.titulo, cover1: data.urlPortada1, cover2: data.urlPortada2, cover3: data.urlPortada3, version: data.versionActual, description: data.descripcion, shortDescription: data.descripcionCorta, price: data.precio, genres: data.generos, tags: data.tags, languages: data.idiomas, warnings: data.avisos}
}
